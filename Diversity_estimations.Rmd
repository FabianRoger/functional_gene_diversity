...
# change 'var' to something else to invalidate all cched chunks (i.e. force to recompute them) 
...
```{r, cache = TRUE}
meta <- read.table("metadata_old_new.txt") %>% filter(season == "summer")
```
...
RareDF_nifh_df %>% 
  left_join(meta) %>% 
  na.omit() %>% 
  ggplot(aes(x = m, y = qD, colour = sediment))+
