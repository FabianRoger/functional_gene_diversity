...
+ Level 4 - replicate A - habitat sand, cyano & ruppia
+ Level 4 - replicate D - habitat cyano

# Import and clean metadata and functional data

```{r, message = FALSE}
meta <- read.table("metadata_old_new.txt")
...
VST_nif <- VST_nif + abs(min(VST_nif))

NMDS_nif <- metaMDS(t(VST_nif), autotransform = FALSE, trace = 0, distance = "bray")

NMDS_plot_nifh <- NMDS_nif$points %>% 
  data.frame() %>% 
  rownames_to_column(var = "sample") %>% 
  mutate(sample = as.numeric(sample)) %>% 
  left_join(.,FuncDat) %>%                     ### replace with updated FuncDat ???
  ggplot(aes(x = MDS1, y =MDS2, colour = sediment))+
  geom_text(aes(label = sample), nudge_x = 0.05)+
  geom_point()+
  theme_bw()+
...
VST_nosz1 <- VST_nosz1 + abs(min(VST_nosz1))

NMDS_nosz1 <- metaMDS(t(VST_nosz1), autotransform = FALSE, trace = 0, distance = "bray")

NMDS_plot_nosz1 <- NMDS_nosz1$points %>% 
  data.frame() %>% 
  rownames_to_column(var = "sample") %>% 
  mutate(sample = as.numeric(sample)) %>% 
  left_join(.,FuncDat) %>%                   ### check
  ggplot(aes(x = MDS1, y =MDS2, colour = sediment))+
  geom_text(aes(label = sample), nudge_x = 0.05)+
  geom_point()+
  theme_bw()+
...
L <- split(meta.t, list(meta.t$sediment, meta.t$season))

DistSum <- lapply(L, function(x) {dd <- D[as.character(x$sample), as.character(x$sample)]; rowSums(dd) })
...
unlist(DistSum) %>% 
  ...
  mutate(sample = gsub(".+?(\\d+)", "\\1", name)) %>% 
  group_by(ID) %>% 
  mutate(mean = mean(distsum)) %>% 
  mutate(sd = sd(distsum)) %>% 
  mutate(limit = mean + 1*sd) %>% 
  filter(distsum > limit) %>% 
  ungroup() %>% 
  select(sample)'
...
'points <- points %>% 
  #mutate(label = ifelse(sample %in% as.numeric(outlier.samples$sample), as.character(sample), ""))
  mutate(label = ifelse(sample %in% c(56,59,68), sample, ""))'
